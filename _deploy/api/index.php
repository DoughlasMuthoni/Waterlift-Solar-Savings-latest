...
{
    // Step 1: swap {id} → a sentinel that preg_quote will not mangle
    $sentinel = 'IDPLACEHOLDER999';
    $withSentinel = str_replace('{id}', $sentinel, $pattern);

    // Step 2: escape all regex metacharacters
    $escaped = preg_quote($withSentinel, '#');

    // Step 3: restore the named capture group
    $regex = str_replace($sentinel, '(?P<id>[0-9]+)', $escaped);
...
foreach ($routes as $route) {
    ...
    // Call the method — pass numeric id as first argument when present
